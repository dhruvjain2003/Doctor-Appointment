...
function AddDoctor() {
    ...
    const validateForm = () => {
        ...
        if (!doctorData.consultation_fee || isNaN(doctorData.consultation_fee) || doctorData.consultation_fee < 0) {
            newErrors.consultation_fee = "Consultation fee must be a valid number (≥ 0)";
        }
        if (doctorData.contact_number && !/^\d{10}$/.test(doctorData.contact_number)) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (doctorData.contact_number) formData.append("contact_number", doctorData.contact_number);
